function flag = GJK_2D(shape1,shape2,iterations)
...
function [a,b,c,flag] = pickTriangle(a,b,shape1,shape2,IterationAllowed)
    ...
    b = a;
    a = support(shape2,shape1,v);
    if round(dot(a,v),4)<0
        flag = 0;
        return;
    end
    if sum(round(b-a,4))==0
        flag = 0;
        return;
    end
